...
from ai_code_summary.files.file_manager import clear_tmp_folder, get_code_files, read_file, write_files_to_tmp_directory, get_tree
...
def create_markdown_from_code(directory: str, exclude_gitignore_dirs: list[str] = _EXCLUDE_GITIGNORE_DIRS) -> None:
    ...
    tree = get_tree(folder_path=directory)
    ...
    write_files_to_tmp_directory(directory, spec, base_dir, output_temp_code_dir,tree)

    code_files = get_code_files(output_temp_code_dir, pathspec.PathSpec([]))
    file_contents = [read_file(file_path , tree ,append_tree=False) for file_path in code_files]
...
def _write_markdown_file(file_info: Tuple[Path, str], base_dir: Path, output_file_name: Path) -> None:
    ...
    relative_path = content.strip().splitlines()[0]
    logger.info(f"")
    with open(output_file_name, "a") as f:
        f.write(f"## {relative_name.name}\n\n")
